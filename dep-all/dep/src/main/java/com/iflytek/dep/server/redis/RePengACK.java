...
@Service
public class RePengACK {

    private Logger logger = LoggerFactory.getLogger(RePengACK.class);
    ...
    {
        rePendingScheduled( RedisQueueType.PENG_DOWN_ACK.getCode(),RedisQueueType.DOING_DOWN_ACK.getCode(), DownloadTaskStatus.DOWN_DEP_DOING.getName(), DownloadTaskStatus.DOWN_DEP_PENGING.getName());
...
//                String nodeId = fileInfo.getCurNodeId();
                ...
                //任务开始后半小时还没完成
                if(status.equals(status_old) && ((dTime/1000) > 2)){
                    try {
