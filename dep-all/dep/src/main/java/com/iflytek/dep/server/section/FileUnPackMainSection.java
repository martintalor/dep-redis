...
@Service
public class FileUnPackMainSection implements Section, Status {
    @Autowired
    ...
    private static Logger logger = LoggerFactory.getLogger(FileUnPackMainSection.class);
    ...
    public void doAct(final String pkgId, String jobId, SectionNode sectionNode, BigDecimal totalSectionNumber, ConcurrentHashMap<String, Object> map) throws Exception {
        ...
        // 解压，非中心节点则不解压
        boolean isVaild = unPack(pkgId,  jobId,  sectionNode,  totalSectionNumber,  map);
    ...
    private boolean unPack(final String pkgId, String jobId, SectionNode sectionNode, BigDecimal totalSectionNumber, ConcurrentHashMap<String, Object> map) {
        //整理更新参数
        ...
        String curNode = FileConfigUtil.CURNODEID;// 当前节点
        try {
...
//            ConcurrentHashMap<String, Object> paramMap = gson.fromJson(gson.toJson(param.get(0)), ConcurrentHashMap.class);
            ...
            // 如果不需要在中心解压完成
            if (null == map.get("UNPACK_MAIN") || !"TRUE".equals( String.valueOf(map.get("UNPACK_MAIN")) )) {
                // 当作解压完成
                return true;
            }


            String packPath = String.valueOf(map.get("UNPACK_MAIN_PATH"));
