...
@Service
public class RePengPkg {

    private Logger logger = LoggerFactory.getLogger(RePengPkg.class);
    ...
    {
        rePendingScheduled(RedisQueueType.PENG_UP_PKG.getCode(),RedisQueueType.DOING_UP_PKG.getCode(), UploadTaskStatus.UP_DEP_DOING.getName(),UploadTaskStatus.UP_DEP_PENGING.getName());
        rePendingScheduled(RedisQueueType.PENG_DOWN_PKG.getCode(),RedisQueueType.DOING_DOWN_PKG.getCode(), DownloadTaskStatus.DOWN_DEP_DOING.getName(),DownloadTaskStatus.DOWN_DEP_PENGING.getName());
...
//                String nodeId = fileInfo.getCurNodeId();
                ...
                if(status.equals(status_old) && ((dTime/1000) > maxExecuteTimeByMinute)){
                    try {
                        ...
                        {
                            String ret = redisUtil.rePengingTask(pengingKey + "_" +  level ,doingKey + "_" +  level,packageId ,status_old,status_new,dateTimeStr);
